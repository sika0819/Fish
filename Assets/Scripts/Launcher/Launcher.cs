...
public class Launcher
{
    ...
    public float Angle {
        get {
            ...
            m_Angle = value;
            //Debug.Log(value);
            m_GunBarrel.UpdateAngle(value);
            angle = Utility.FloatToShort(value);
  ...
  //      Debug.Log(launcherType);
    ...
    public void Update(float delta)
    {
        ...
        {
            //向服务器发送大招请求
            //short angle = Utility.FloatToShort(Angle);
            //angle = SceneRuntime.AngleInversion(angle);
        ...
        if (CheckLaunch())
        {
            ...
            if (m_LauncherTime >= m_LauncherInterval)
            {
                ...
                m_LauncherTime = 0;
                //检测是否钱够
                // Debug.Log("现在有："+ PlayerRole.Instance.GetPlayerGlobelBySeat(m_Seat));
                //    Debug.Log(BulletSetting.BulletRate[m_RateIndx] * m_LauncherSetting.Consume);
    ...
    public void UpdateLaunchByLockFish(Vector3 screePos)
    {
        ...
        Angle = Mathf.Acos(Mathf.Clamp(dot, 0.0f, 1.0f)) * Mathf.Rad2Deg;
        //第一三象和第二四象角度相反
        if (m_Direction.x >= 0)
            Angle = -Angle;
    ...
    public void UpdateEnergy(int energy)
    {
        ...
        m_EnergyPoolLogic[m_Type].UpdateEnergyPool(m_Type, m_RateIndx,(uint)energy);
        m_EnergyPoolUI.fillAmount = m_EnergyPoolLogic[LauncherType].FillAmount;
    }
    //非自己炮台角度更新
    public void UpdatOtherAngle()
    {
        ...
        if (m_Direction.x >= 0)
            Angle = -Angle;
        Angle = Mathf.Clamp(Angle, -85, 85);
        m_GunBarrel.UpdateAngle(m_Seat > 1 ? -Angle : Angle);
    }
    ...
    public void LaunchLaser(byte clientSeat, short degree)
    {
        ...
        SceneRuntime.LauncherEftMgr.PlayLaserEft(m_GunBarrel.BaseTransform, LauncherType, clientSeat, Direction, startPos, Angle);
       // SceneRuntime.LauncherEftMgr.RemoveAtEffect();
    }
