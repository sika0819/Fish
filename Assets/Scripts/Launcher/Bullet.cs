...
public class Bullet:IGlobalEffect
{
    ...
    {
        bulletPool = PoolManager.Pools["bullet"];
        bulletPairPool = PoolManager.Pools["bulletPair"];
        GameObject m_tempObj = (GameObject)SceneRuntime.BulletMgr.BulletObj[launcherType];
        m_Object = bulletPool.Spawn(m_tempObj, startPos, Quaternion.identity).gameObject;
        ...
        m_Pos           = startPos;
        m_Dir           = dir;
        GameObject obj      =(GameObject)SceneRuntime.BulletMgr.GetBulletPar(launcherType);
        //m_BulletParEft  = (GameObject)GameObject.Instantiate(obj, startPos, Quaternion.identity);
        m_BulletParEft = bulletPairPool.Spawn(obj, startPos, Quaternion.identity).gameObject;
    ...
    public void Destroy()
    {
        ...
        //}
        if(bulletPool.IsSpawned(m_Trans))
            bulletPool.Despawn(m_Trans);
