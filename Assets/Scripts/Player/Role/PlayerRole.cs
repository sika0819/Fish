...
class PlayerRole : Singleton<PlayerRole>
{
    ...
    {
        //计算需要扣除的金币的数量
       SceneRuntime.ClientToServerSeat(Seat);//客户端转服务器端的位置
    }
    ...
    {
        //因为捕获鱼 添加金币
         Byte TempSeat = SceneRuntime.ClientToServerSeat(Seat);
         bool IsInMonth = (PlayerRole.Instance.RoleInfo.RoleMe.GetMonthID() != 0);
         if (PlayerRole.Instance.RoleInfo.RoleMe.GetSeat() == TempSeat)
         {
         ...
         else
         {
             ...
             else
             {
                ...
                if (pTableRole != null)
                {
                    if (GlobelSum < 0 && pTableRole.GetGlobel() < -1 * GlobelSum)
                        return false;
                    if (pTableRole == null)
                        return false;
