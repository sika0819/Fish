...
public class SceneBulletMgr
{
    ...
    Object[] m_bulletPreb = new Object[ConstValue.MAX_LAUNCHER_NUM];
    Object[]    m_BulletParObj = new Object[ConstValue.MAX_LAUNCHER_NUM];
    SpawnPool bulletPool;
    SpawnPool bulletPairPool;
 //   uint        m_BulletCount = 0;
    public void Init()
    {
        ...
        for (int j = 0; j < m_bulletPreb.Length; ++j)
        {
            string str = string.Format("LauncherBullet{0}", j);
            m_bulletPreb[j] = ResManager.Instance.LoadObject(str, "SceneRes/Prefab/Effect/Launcher/Bullet/", ResType.SceneRes);
        ...
        if (!PoolManager.Pools.ContainsKey("bullet")) {
            bulletPool= PoolManager.Pools.Create("bullet");
            bulletPool.poolName = "bullet";
        }
        if (!PoolManager.Pools.ContainsKey("bulletPair"))
        {
            bulletPairPool=PoolManager.Pools.Create("bulletPair");
            bulletPairPool.poolName = "bulletPair";
        }
    }
    public Object[] BulletObj
    {
        get { return m_bulletPreb; }
        set { m_bulletPreb = value; }
    }
    ...
    public void Shutdown()
    {
        ...
        for (byte j = 0; j < m_bulletPreb.Length; ++j)
        {
            ResManager.Instance.UnloadObject(m_bulletPreb[j]);
            ResManager.Instance.UnloadObject(m_BulletParObj[j]);
        }
        PoolManager.Pools.Destroy(bulletPool.poolName);
        PoolManager.Pools.Destroy(bulletPairPool.poolName);
