...
public class SceneFishMgr
{
    ...
    public void Init()
    {
        if(!PoolManager.Pools.ContainsKey("Fish"))
            FishPool = PoolManager.Pools.Create("Fish");
        FishPool = PoolManager.Pools["Fish"];
        FishPool.poolName = "Fish";
    }
...
#endif
                fish.SetVoice(cmdFish.randIndex);
    ...
    public void Shutdown()
    {
        ...
        foreach(Fish f in m_FishList.Values)
            f.Destroy();
        m_FishList.Clear();
        PoolManager.Pools.Destroy("Fish");
    }
