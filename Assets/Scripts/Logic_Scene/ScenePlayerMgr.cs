...
public class ScenePlayerMgr
{
    ...
    public void Init()
    {
        ...
        m_launcherEffectObj = ResManager.Instance.LoadObject("MyLauncherEffect", "SceneRes/Prefab/UI/Launcher/", ResType.SceneRes);
    }
    ...
    public Object LauncherEffectObject
    {
        get {
            return m_launcherEffectObj;
        }
    }
    ...
    public bool PlayerJoin(PlayerExtraData player, byte clientSeat, byte rateIdx, byte launcherType, bool valid)
    {
        ...
        InstallPlayerEffectOnce(PlayerCount);
    ...
    void InstallPlayerEffectOnce(int playerCount)
    {
        if (playerCount > 1&&(m_effectOnce==false))
        {
            m_effectOnce = true;
            PlayerEffect = GameObject.Instantiate(LauncherEffectObject) as GameObject;
            PlayerEffect.transform.SetParent(SceneObjMgr.Instance.UIPanelTransform, false);
            PlayerEffect.transform.position = MySelf.Launcher.LauncherPos;
            if (PlayerEffect)
                GameObject.Destroy(PlayerEffect, 20);
        }
    }
    ...
    public void Shutdown()
    {
        ...
        if (PlayerEffect)
            GameObject.DestroyImmediate(PlayerEffect);
        m_effectOnce = false;
        m_bAutoShot = false;
    }
