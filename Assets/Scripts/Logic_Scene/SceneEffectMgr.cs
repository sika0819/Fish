...
public class SceneEffectMgr
{
    ...
    {
        if(main)
            return EffectPool.Spawn((GameObject)m_LightingFishEffect1).gameObject;// return (GameObject)GameObject.Instantiate(m_LightingFishEffect1);
        else
            return EffectPool.Spawn((GameObject)m_LightingFishEffect2).gameObject;
        // return (GameObject)GameObject.Instantiate(m_LightingFishEffect2);
    }
    public GameObject GetSkillEffect(ushort idx)
    {
        return EffectPool.Spawn((GameObject)m_SkillEffectList[idx]).gameObject;
    ...
    {
        return EffectPool.Spawn((GameObject)m_LaserSubEffectList[idx]).gameObject;
    ...
    public void LoadBackEffect(int backidx)
    {
        if (m_BackEffectMapping[backidx] != null)
        {
            ...
            for (int i = 0; i < m_LaunchEffectList.Length; ++i)
            {
                ...
                if (m_BackEffectData[idx].EffectObj != null)
                {
                    GameObject EffectObj = EffectPool.Spawn((GameObject)m_BackEffectData[idx].EffectObj).gameObject;
    ...
    public void ClearBackEffect()
    {
        if (m_LaunchEffectList != null)
        {
            for (int i = 0; i < m_LaunchEffectList.Length; ++i)
            {
                //if (EffectPool.IsSpawned(m_LaunchEffectList[i].transform))
                //    EffectPool.Despawn(m_LaunchEffectList[i].transform);
    ...
    public void Shutdown()
    {
        ...
        PoolManager.Pools.Destroy("Effect");
    }
    ...
    public void FishCatched(Fish fish, CatchedData cd)
    {
        ...
        m_GoldEffect.PlayAudio(fish.FishType,fish.randIndex);
        if (fish.IsBossFish())
        {
            //BossFish彩蛋特效
        ...
        if (fish.IsBigFish())
        {
            m_GoldEffect.ShowBigFishGoldEffect(cd, fish);
        }
    ...
    public void PlayFishNet(Vector3 bulletPos , byte bulletType)
    {
        ...
        {
            GameObject Gobj = EffectPool.Spawn((GameObject)m_FishNetObj[bulletType]).gameObject;
