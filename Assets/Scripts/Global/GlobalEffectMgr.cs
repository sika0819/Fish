...
public class GlobalEffectMgr:Singleton<GlobalEffectMgr>
{
    ...
    public void Clear()
    {
        if (PoolManager.Pools.ContainsKey("Effect"))
        {
            ...
            //    if (EffectPool.IsSpawned(m_EffectList[i].EffectObj.transform))
            //        EffectPool.Despawn(m_EffectList[i].EffectObj.transform);
            //}
    ...
    public void Update(float delta)
    {
        ...
        for(int i = 0; i < m_EffectList.Count; )
        {
            ...
            if(effect.IsEnd)
            {
                ...
                if (EffectPool!=null)
                {
                    if (EffectPool.IsSpawned(effect.EffectObj.transform))
                    {
                        EffectPool.Despawn(effect.EffectObj.transform);
                    }
