...
class GlobalAudioMgr : Singleton<GlobalAudioMgr>
{
    ...
    public void PlayVoiceMusic(Audio.EffectVoiceType voiceIndex, bool bLoop = false/*是否循环*/, bool bStopLoop = false/*是否停止循环主要是用来听上个音效*/, float volume = 1)
    {
        if (bStopLoop)
        {
            if (LoopAudio == null)
