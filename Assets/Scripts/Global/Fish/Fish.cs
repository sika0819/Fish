...
public class Fish:IGlobalEffect
{
  ...
  Vector3         m_ViewPos;
    ...
    {
        FishPool = PoolManager.Pools["Fish"];
        ...
        GameObject temp = (GameObject)FishResManager.Instance.GetFishObj(type);
        m_Model = FishPool.Spawn(temp,temp.transform.position,temp.transform.localRotation).gameObject;
        ...
        if (type == 0) {
            ...
            ParticleSystem[] originps = temp.GetComponentsInChildren<ParticleSystem>();
            for (int i = 0; i < ps.Length; ++i)
            {
                ps[i].startSize = originps[i].startSize;
                ps[i].Clear();
            }
        }
        SetScaling(scl);
    ...
    public void Destroy()
    {
        ...
        //FishPool.Despawn(m_ModelTransform);
    ...
    {
        return m_FishType == 1 || m_FishType == 3 || m_FishType == 19 || m_FishType == 9 || m_FishType == 18 || m_FishType == 20||m_FishType==13||m_FishType==10;
    }
}
